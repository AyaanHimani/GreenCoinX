...
const ProducersLeaderboard = ({ setCurrentView }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get("api/leaderboard/getAllProducers");
        setProducersData(res.data || []);
      } catch (err) {
        console.error("Error fetching producers leaderboard:", err);
      }
    };
    fetchData();
  }, []);

  const totalPages = Math.ceil(producersData.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = producersData.slice(startIndex, endIndex);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white/80 p-6 rounded-2xl shadow-lg border">
            ...
            <p className="text-sm text-gray-600">Total Producers</p>
            <p className="text-2xl font-bold">{producersData.length}</p>
          </div>
          <div className="bg-white/80 p-6 rounded-2xl shadow-lg border">
            <TrendingUp className="w-8 h-8 text-emerald-600" />
            <p className="text-sm text-gray-600">Top Performer</p>
            <p className="text-2xl font-bold">
              {formatValue(producersData[0]?.totalHydrogenProducedKg || 0)}
            </p>
          </div>
          <div className="bg-white/80 p-6 rounded-2xl shadow-lg border">
            <Star className="w-8 h-8 text-emerald-600" />
            <p className="text-sm text-gray-600">Avg GreenCoins</p>
            <p className="text-2xl font-bold">
              {(
                producersData.reduce(
                  (sum, d) => sum + (d.greenCoinsMinted || 0),
                  0
                ) / (producersData.length || 1)
              ).toFixed(1)}
            </p>
          </div>
        </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className="font-bold text-lg">
                        {formatValue(item.totalHydrogenProducedKg)}
                      </div>
        ...
        <div className="mt-8 flex justify-between items-center">
          <div className="text-sm text-gray-600">
            Showing {startIndex + 1}-{Math.min(endIndex, producersData.length)} of{" "}
...
export default ProducersLeaderboard;
